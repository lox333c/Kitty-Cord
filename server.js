...
// --- 1. НАСТРОЙКА CLOUDINARY (КАРТИНКИ И ФАЙЛЫ) ---
cloudinary.config({
...
const storage = new CloudinaryStorage({
    ...
    params: {
        ...
        allowed_formats: ['jpg', 'png', 'gif', 'jpeg', 'webm', 'mp4', 'mp3', 'wav', 'ogg']
    },
});
...
// --- 3. ВСЕ ТВОИ API МАРШРУТЫ (АДАПТИРОВАННЫЕ ПОД TURSO) ---

app.post('/api/upload', upload.single('file'), (req, res) => {
    if (!req.file) return res.status(400).json({ error: 'Нет файла' });
    res.json({ success: true, url: req.file.path }); // Прямая ссылка из Cloudinary
});
...
app.get('/api/dms/:username', async (req, res) => {
    ...
    const dms = new Set();
    if (rs.rows) rs.rows.forEach(r => { if (r.sender !== 'general' && r.recipient !== 'general' && !r.recipient.startsWith('channel_')) dms.add(r.sender === u ? r.recipient : r.sender); });
